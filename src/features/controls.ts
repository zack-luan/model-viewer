...
import {parseValues} from '../value-parsing.js';

const positionStringToOrbit = (positionString: string): Array<number> => {
  const cameraPositionValues = parseValues(positionString);

  if (cameraPositionValues.length === 3) {
    const [azimuthNode, poleNode, radiusNode] = cameraPositionValues;

    const azimuth = parseFloat(azimuthNode.value as any);
    const pole = parseFloat(poleNode.value as any);
    const radius = parseFloat(radiusNode.value as any);

    const theta =
        azimuthNode.unit === 'deg' ? azimuth * Math.PI / 180 : azimuth;

    const phi = poleNode.unit === 'deg' ? pole * Math.PI / 180 : pole;

    let radiusScale;

    switch (radiusNode.unit) {
      default:
      case 'm':
        radiusScale = 1;
        break;
    ...
    const radiusMeters = radius * radiusScale * FRAMED_HEIGHT;

    return [theta, phi, radiusMeters];
  }

  return positionStringToOrbit(DEFAULT_CAMERA_POSITION);
};

/*const orbitToPositionString =
    (theta: number, phi: number, radius: number): string => {
      return `${theta}rad ${phi}rad ${radius}m`;
    };*/


const DEFAULT_CAMERA_POSITION = '0deg 75deg 1.7m';
...
const $cameraPosition = Symbol('cameraPosition');
...
export const ControlsMixin = (ModelViewerElement:
    ...
    Constructor<ModelViewerElementBase> => {
      class ControlsModelViewerElement extends ModelViewerElement {
        ...
        protected[$cameraPosition]: string = DEFAULT_CAMERA_POSITION;
        ...
        constructor() {
          ...
          this.requestUpdate('cameraPosition');
        }
        ...
        set cameraPosition(value: string) {
          const oldValue = this[$cameraPosition];
          this[$cameraPosition] = value;
          this.requestUpdate('cameraPosition', oldValue);
        }
        ...
        updated(changedProperties: Map<string, any>) {
          ...
          if (changedProperties.has('cameraPosition')) {
            controls.setOrbit(...positionStringToOrbit(this.cameraPosition));
          }
        }
        ...
        [$updateOrbitCamera]() {
          ...
          this[$controls].applyOptions({maximumRadius: camera.position.z * 2});
        }
